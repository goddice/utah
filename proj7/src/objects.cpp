    ...
    if (use_bvh) {
        ...
        while(!bfs.empty()) {
            ...
            if (bvh.IsLeafNode(id)) {
                res |= TraceBVHNode(ray, hInfo, hitSide, id);
            }
            else {
                if (TraceBVHNode(ray, hInfo, hitSide, id)) {
                    bfs.push(bvh.GetFirstChildNode(id));
                    bfs.push(bvh.GetSecondChildNode(id));
                }
            }
        }
        return res;
        ...
        bool res = false;
        for (int i = 0; i < nf; ++i) {
            res |= IntersectTriangle(ray, hInfo, hitSide, i);
        }
        return res;
    }
}
...
bool TriObj::TraceBVHNode( const Ray &ray, HitInfo &hInfo, int hitSide, unsigned int nodeID ) const {
    Box b(bvh.GetNodeBounds(nodeID));
    if (b.IntersectRay(ray, BIGFLOAT)) {
        if (bvh.IsLeafNode(nodeID)) {
            unsigned int nf = bvh.GetNodeElementCount(nodeID);
            const unsigned int* fids = bvh.GetNodeElements(nodeID);
            bool res = false;
            for (int i = 0; i < nf; ++i) {
                res |= IntersectTriangle(ray, hInfo, hitSide, fids[i]);
            }
            return res;
        }
        else {
